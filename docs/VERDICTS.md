...
修复否定关键词误报、关键词堆砌误通过、逐用例参考程序异常漏检；增加人工复核导出与指纹校验汇总、分层等量选题及 Wilson 区间。详见 [评估与人工复核流程](EVALUATION_WORKFLOW.md)。

## 兼容与发布

本次保留原布尔字段，并加入可空状态、覆盖统计及说明，不改数据库表结构。旧 JSON 任务不会自动改判，网页将缺少覆盖数据的旧记录标为历史信息；请重新发起评估验证新规则。重启 Web 和独立 Worker 后刷新页面。此前代码提交功能要求的 `0003` 迁移仍需完成，但本次无需新增迁移。
