...
模型评测首页保留 Hy3 生成与多 Agent 评估流程；独立的 `/submit` 页面直接验证用户代码，不调用 Solver，也不替用户编写推理步骤。

## 操作方式

1. 从首页点击“提交我的代码”进入 `/submit`，再从左侧选择题库中对应的题目。题面、输入格式、约束与公开示例仍可查看。
...
- 6 道原创题和 65 道外部题使用逐题审核的显式 Hypothesis 策略；报告记录实际策略版本、输入域和执行状态。

## 安全环境与数据库升级

**生产环境中的用户代码任务必须使用 Docker。** 未配置 Docker、镜像不存在或服务不可用时，生产接口拒绝入队；Worker 执行时再次检查。

本机演示可以显式启用受限本地沙盒，但仅在 `APP_ENV` 不是 `production` 且 Web 服务绑定 `127.0.0.1`、`localhost` 或 `::1` 时开放。该模式不是宿主机安全边界，只能运行自己信任的代码：
