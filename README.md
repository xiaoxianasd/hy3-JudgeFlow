...
## 当前更新：可恢复基准评测与完整属性测试覆盖（2026-09-07）
...
## 上次更新：首步误判修复与用户代码提交（2026-09-03）

### 1. 修复“几乎所有题目都被判为第一步错误”的已知问题
...
- **独立入口**：网页顶部切换“Hy3 自动解题 / 提交我的代码”；代码草稿按题目分别保存在页面内存中，避免切题时混用。
...
- **MySQL 用户必须先升级数据库**：停止旧 Web/Worker，执行启动器菜单 **7** 或 `Hy3_TraceJudge.bat database`，将结构升级到 `0003`，再使用菜单 **1** 启动 Web。迁移仅增加可空的 `submission_json` 字段，不删除历史任务；部署前建议备份数据库。
...
- **用户代码提交**：网页独立“提交我的代码”工作区，选择题目、粘贴 Python 实现，可选附上每行一步的解题说明；分别报告测试结果、实现逻辑、推理过程、错误代码行或步骤。用户代码强制使用 Docker 沙盒。见 [代码提交使用说明](docs/CODE_SUBMISSIONS.md)。
...
| 去重构造集与对抗轨迹、人工复核导出/汇总 | 已实现；真实人工结论待填写 |
...
浏览器打开 `http://127.0.0.1:8765`。在“Hy3 自动解题”分区选择题目和编排模式后运行；在“提交我的代码”分区填写自己的代码与可选解题步骤。后者固定使用一次专用审查，不使用生成模式的 Supervisor/Swarm 选项。
