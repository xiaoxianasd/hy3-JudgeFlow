...
- **用户代码提交**：独立 `/submit` 页面用于选择题目、粘贴 Python 实现，并可选附上每行一步的解题说明；分别报告测试结果、实现逻辑、推理过程、错误代码行或步骤。用户代码强制使用 Docker 沙盒。见 [代码提交使用说明](docs/CODE_SUBMISSIONS.md)。
...
| 用户代码提交与可选步骤审查 | 已实现；启用须配置 Docker 与升级数据库 |
| 6 道 easy / medium / hard 种子题 | 已实现并完成五维量表校准，每档2题 |
| 60 道 MBPP / EvalPlus 外部题 | 已接入并配置属性测试策略；与种子题合计 66 道 |
| 去重构造集与对抗轨迹、人工复核导出/汇总 | 已实现；新增正确代码＋错误证明及条件推广样本，受控与自然结果分开统计 |
| 大规模真实 Hy3 评测与新提交链路部署验收 | 待完成 |
...
界面将两类证据分开显示：绿色的“代码验证证据”只说明当前测试覆盖内的程序行为；红色的“推理反例证据”显示支持首错结论的步骤、反例、Agent 来源和置信度。完整推理、多 Agent 审查和生成代码默认折叠，便于先核对结论及直接证据。
...
2026-09-07 使用 `hy3`、Supervisor 五专家和每题60次 Hypothesis 预算，对人工量表校准的6道种子题进行等量分层试运行。自动评估正确率为4/6（66.67%）；复核者 `xiaoxianasd` 完成6/6人工复核后，纠正1条沙盒执行约束误报，最终答案与过程正确率均为5/6（83.33%）。错误答案上的首错定位为1/1，正确过程误报率为1/5。该误报暴露的契约问题现已修复：生成提示和执行器共用安全导入白名单，结果也区分模型、评估器与基础设施归属。

easy、medium 的人工复核正确率均为2/2，hard 为1/2。每层只有2题且 Wilson 区间高度重叠，因此只能记录 hard 层下降信号，不能确认模型能力临界点。完整数据、指标口径和典型案例见 [真实 Hy3 分层过程评估报告](reports/hy3_seed_stratified_20260907_report.md)。

本轮 P0 修复与证据展示回归：Python **215 passed / 2 skipped**，Node 判定测试 **11 passed**；模型切换页面已通过本地浏览器可视验收。历史 BFS 误报样例在当前沙盒运行器本地后端通过6/6固定测试和60次属性测试；本机没有 Docker CLI，Docker 镜像重建与端到端重放需在部署环境执行。
...
网页左侧“模型设置”可在 `hy3` 与 `hy4-preview` 之间逐任务切换，也可填写 TokenHub API Key。“保存并检查模型”会执行一次小型真实推理，而不只检查模型列表，因此能提前识别 401/403、额度不足和 HTTP 429 限流。选择 `hy4-preview` 时默认使用 Single 单评审以减少预览模型的并发调用，仍可在高级设置中手动改回多 Agent。模型 Key 仅保存在当前浏览器标签页和任务进程内存中，不写入任务数据库、结果或状态响应；留空时使用服务端 `.env` 中的 `HY3_API_KEY`。MySQL 队列只持久化所选模型名和临时凭据标记。若服务在任务完成前重启，浏览器 Key 会按设计失效，任务明确报错并要求重新提交。
...
当前已接入网页的题库为 **66 道**：6 道原创题与 60 道通过 EvalPlus 导入的 MBPP 题。下表中的其他数据集是扩展规划或研究参考，不表示已经全部集成，也不用于训练模型。
...
- **诚实展示测试覆盖**：Hypothesis 分别显示未配置、未执行、验证异常、发现反例、预算内未发现反例；展示实际检查次数及评审完成数。此条记录的是当时状态；当前 66 道题均已配置明确策略。
...
- **用户代码执行必须启用 Docker**：构建沙盒镜像并配置 `SANDBOX_BACKEND=docker`。即使是开发模式，也不允许用户代码回退到本机执行；Docker 未就绪时提交会被拒绝。
